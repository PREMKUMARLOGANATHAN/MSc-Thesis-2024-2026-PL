...
def ccm_result_1(func, X, Y, L_range, tau_y, tau_x, E_y, E_x):
    ...
    ax.plot(L_range, Xhat_My, label='AMR causes Precipitation', linewidth = 1.5)
    ax.plot(L_range, Yhat_Mx, c='r', label='Precipitation causes AMR', linewidth = 1.5)
...
def extended_ccm(func, X, Y, L, tau_y, tau_x, E_y, E_x, tp):
    Xhat_My, Yhat_Mx = [], []
    for T in np.arange(-tp, tp + 1, 1):
